...
- src/monthly_check.py: 5부문 자동 점검 (무결성/체결/성과/포트폴리오/학습판단)
  - 판정 규칙: 부문 1·2 실패 = "운영 이상 — 복구 우선(학습 보류)" (성과보다 생존)
  - maker 비중/슬리피지는 체결 누적 후 측정 항목으로 정직 표기
- runner 통합: 매월 1일 자가학습 직전 실행 → results/monthly_report_YYYY-MM.json
- docs/OPERATIONS.md 월간 섹션 추가 (실행 순서 A→B→C, 월간 한 줄 원칙)
- pytest 75 passed, 러너 재시작 (현 시점 수동 실행 판정: ✅ 생존 점검 통과)

## [2026-06-11 18:40] Iteration 33 — 운영 규칙서 제정 + 코드화 (docs/OPERATIONS.md)
- 외부 리뷰의 교체 허용 조건표 12항목 갭 분석: 9개 기구현 확인, 미구현 3개 코드화
  ① 90일 이후에도 드리프트 정상이면 교체 금지 (평상시 보수 유지 — B규칙 정합)
  ② 체결/잔고 이상(recon_block/halted) 시 학습 보류 (운영 리스크 우선)
  ③ src/gate_check.py — 90일 관측 종료 체크리스트 5부문 자동 평가 (현재 판정: ⛔ 관측 유지, 정상)
- A/B/C 의사결정 규칙·체크리스트·한줄 원칙을 docs/OPERATIONS.md로 명문화
- pytest 73 passed (정책 테스트 2종 추가)

## [2026-06-11 18:10] Iteration 32 — 방향 재정의 반영: 보수적 적응형 생존 시스템 (리뷰 1~3단계 집행)
- 2단계 기여 분해 (docs/CONTRIBUTION.md) — **리뷰 가설 1건 데이터로 기각**:
  - BB는 횡보 완충재 아님 → 상승장 증폭기 (uptrend +94%/yr, 위기엔 동반 하락 LUNA -46%)
  - 진짜 위기 보험 = sma(10,200): 단독 Sharpe 0.24로 희석원이지만 제외 시 MDD -38→-44% (코로나 +53.5%)
  - st = 수익 엔진 (제외 시 Sharpe 1.06→0.72). 2025 개선의 원천 = 정적 분산이 아니라 적응(재선택)
- 3단계 자가학습 보수화: ① 라이브 90일 미만+비CRITICAL = 관측 모드(교체 금지)
  ② 북당 연속 2회 교체 시 쿨다운. 테스트 3종 추가 → pytest 71 passed
- 1단계 문서 3북 통일 + 4/5단계 게이트 STATE 기록 (PSR>0.5, 드리프트 하위25% 탈출, 체결 품질)
