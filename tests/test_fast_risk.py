...
def test_stop_loss_fires_intraday(tmp_path, monkeypatch):
    """진입가 대비 -4% 도달 → 즉시 청산 + 재진입 블록."""
    setup_env(tmp_path, monkeypatch,
              {"sma_slow": long_book(60_000)}, price=57_500, peak=2_500)  # -4.2%
    out = pt.fast_risk_check()
    assert any("손절" in e for e in out["events"])
    st = json.loads((tmp_path / "s.json").read_text())
    assert st["books"]["sma_slow"]["position"] == 0
    assert st["books"]["sma_slow"]["blocked_sign"] == 1


def test_trailing_fires_from_peak(tmp_path, monkeypatch):
    """극값(62,000) 대비 -8% 후퇴 → 트레일링 청산 (진입가 대비론 이익이어도)."""
    setup_env(tmp_path, monkeypatch,
              {"sma_slow": long_book(55_000, extreme=62_000)}, price=56_900, peak=2_600)  # 극값 -8.2%
    out = pt.fast_risk_check()
    assert any("트레일링" in e for e in out["events"])
