...
| 북 | 전략 | 비중 | 설정 |
|---|---|---|---|
| `sma_slow` | SMA 크로스 (10/200) | 40% | 1d, 2.5x, 양방향 |
| `supertrend` | 슈퍼트렌드 (14, 1.5) | 40% | 1d, 2.5x, 양방향 |
| `bb_breakout` | 볼린저 돌파 (20, 2.0) | 20% | 1d, 2.5x, 롱온리 — 고원 9/9 통과 후 2026-06-11 편입 |

3북 합성 (9y 백테스트): **CAGR 37.6%, Sharpe 1.06, MDD -37.6%**

공통: 손절 4% / 트레일링 8% (일봉 종가 평가 — 검증된 정책. 장중엔 -8% 비상밴드만), 변동성 타게팅 연 40% (0.25 계단 분수 포지션),
...
> 표 수치는 2x 기준. **운용 레버리지 2.5x** (9y 백테스트 CAGR 32.2%, Sharpe 0.92, MDD -36.7% —
> Sharpe 불변·-40% 백스톱 정합 확인 후 증액. 3x는 MDD -42.9%로 백스톱 충돌 → 기각).
> 실전 전환 시 추가 레버: 유휴증거금 이자(+3% APY 시 Sharpe 1.00) — 데모에선 미적용(기록 왜곡 방지).
...
```
runner.py (24시간 무인)
├─ 60초   비상 감시 틱 — 장중 -8% 초과 폭주(비상밴드)·킬스위치만 즉시 집행
├─ 15분   신호 사이클 — 캔들 갱신 → 시그널/일봉 스탑 평가 → maker 지정가 주문
...
## 💀 Failed Experiments — 실패 실험 전시관

이 저장소의 신뢰는 채택보다 기각에서 나옵니다. 전부 데이터로 기각:

| 실험 | 결과 | 기각 사유 |
|---|---|---|
| ML 방향예측 (GBT) | -17~-90% | 캔들은 노이즈 > 신호 |
| ML 메타라벨링 | 무효 | 표본 부족 |
| 2y 최적화 듀얼 (자체 전략) | 2y +86% → 9y -27% | 구간 과적합 — 본 프로젝트 이름의 유래 |
| 유명 기법 원형 (터틀/래리 윌리엄스 등) | 전부 OOS 음수 | 원형 복제는 엣지 아님 |
| 펀딩 캐리 북 | OOS 0.0~0.38 | 추세북과 상관 0.36, 분산 효과 없음 |
| 장중 즉시 스탑 | CAGR 21.6→-0.2% | 위크 청산 — 일봉 종가 평가가 정답 |
| 10~20x 레버리지 | 청산 | 손절 4%×20x = 하루 -80% |
| 자산기반 동적 레버리지 4종 | 전부 고정 2.5x에 패배 | DD 감액 = 회복 깎임 |
| 1분봉/4h 진입신호 | OOS -96~-100% | 수수료에 전멸 |
| 재진입 블록 완화 | MDD -67~-90% | 블록 = whipsaw 면역체계 |
