...
공통: 손절 4% / 트레일링 8% (실시간 발동), 변동성 타게팅 연 40% (0.25 계단 분수 포지션), 킬스위치 (자산 고점 -15% 시 전량 청산·중단).

**9년 백테스트** (실측 펀딩비 반영): CAGR +26.4%, Sharpe 0.92, MDD -30.1%, 9년 중 8년 흑자.
**몬테카를로 2,000회**: 손실 확률 1.1%, P(MDD<-50%) 12.3%.
**정직한 한계 (DSR)**: 탐색 강도를 보정하면 통계적으로 우연과 구분 불가 (DSR 0.41~0.75 < 0.95) — 진짜 증거는 라이브 기록으로만 쌓입니다.
...
| `results/paper_trades.csv` | 체결 내역 |
| `results/equity_history.csv` | 자산 곡선 |
| `results/ALERT.txt` | ⚠ 생기면 전략 열화 경고 |
